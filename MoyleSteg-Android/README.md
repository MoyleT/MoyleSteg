# MoyleSteg Android · 0.3.2-alpha

**Kotlin + Jetpack Compose 的 Android 首版源码工程。** 不使用 Python 运行时，不依赖服务器。
PNG／SAES 保持与电脑版 1.4.1 兼容；GIF 动画载体需要电脑版 1.5.0 或后续兼容版本。

> 本版（versionCode 7）增加 **恢复后自动保存到公共 Download 根目录**：PNG／GIF／SAES 认证成功后保存，
> 同名文件不覆盖，结果显示实际保存名称与位置；点击“打开文件”后由系统提供打开方式选择。
> 保存失败可保留当前私有认证成品直接重试。保留 JPG／JPEG 载体、最多 1 GiB SAES、内存设置、GIF 和三主题。
> 最新构建、回归和设备验证边界以 [0.3.2 验证记录](verification/TEST_REPORT_0_3_2.md) 为准；
> [0.3.1 验证记录](verification/TEST_REPORT_0_3_1.md) 保留为历史基线，不代表本版设备验收通过。
...
| 恢复文件保存与打开 | 认证后自动保存 Download，不覆盖同名；显式点击后按类型选择打开应用，失败可重试或另存 |
...
本版未进行手机／模拟器验收，待执行项目见 [设备验收清单](docs/DEVICE_ACCEPTANCE.md)。
...
保留于当前结果，以便重试。清空结果、离开该结果或下次进程启动会清理私有成品；不提供进程终止后的恢复保证。
