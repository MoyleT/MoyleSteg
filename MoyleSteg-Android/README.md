# MoyleSteg Android · 0.4.1-alpha

**Kotlin + Jetpack Compose 的 Android 首版源码工程。** 不使用 Python 运行时，不依赖服务器。
PNG／SAES 保持与电脑版 1.4.1 兼容；GIF 动画载体需要电脑版 1.5.0 或后续兼容版本。

> 本版（versionCode 9）是多文件版的可靠性更新：**临时磁盘预算、文件选择取消、导航与未保存结果保护**。
> 设置往返保留文件列表、当前子功能和认证结果；重复点击当前页不重置任务。切换功能或清空尚未保存的成果前，需要明确确认。
> 继续支持 **一张图片隐藏多个文件**：最多选择 100 个文件，自动打包为带识别标记的标准 ZIP，再统一加密为 PNG／GIF／SAES。
> 恢复时先认证并显示文件清单，选择部分或全部保存到 Download，每项可直接选择应用打开；同名文件不覆盖。
> 取消或失败保留已保存项，剩余项可以重试；也可保存完整 ZIP。普通 ZIP 及单文件继续按原流程自动保存。
> Windows 1.6.0 支持直接选择恢复成员；旧版仍可恢复整个 ZIP 后自行解压。密码学与外层容器 v1 格式保持不变。
> 修复范围见 [0.4.1 可靠性说明](docs/RELIABILITY_0_4_1.md)；最终测试结果记录于 [0.4.1 验证记录](verification/TEST_REPORT_0_4_1.md)。历史记录不代表本版设备验收。

本版交付文件名为 `MoyleSteg-Android-0.4.1-alpha-debug.apk` 与
`MoyleSteg-Android-0.4.1-alpha-Source.zip`；APK 是供验收的调试签名安装包，源码包不包含签名私钥。

## 已编写的能力

| 部分 | 当前状态 |
|---|---|
| AES-256-GCM SAES 加密／解密 | Kotlin 核心已实现；各版本验证结果分别记录 |
...
| 多文件隐写／独立加密 | 多选、添加移除、计数和大小，实际打包预检；最多 100 个文件，保留原名及字节 |
| 恢复文件保存与打开 | 单文件自动保存 Download，多文件先选择；各项独立提交、不覆盖同名、可打开和重试 |
| Android 系统文件选择／导出 | 输入、隐藏成品、独立加密、密钥和另存仍使用 SAF；设备提供方待实机验收 |
| Compose 手机界面与三主题 | 已实现；未进行本版实机渲染验收 |
...
- 多文件同时限制展开后的原文件总量和完整 ZIP 大小，二者都须在上述载荷上限内；ZIP 开销也占容量。
  打包、预检、恢复采用有界流处理 ZIP，但图片处理仍使用完整像素缓冲。需为捕获副本、ZIP、容器和恢复成员留出临时空间。
  多文件流程按阶段检查私有磁盘的新增空间需求，保留 **32 MiB** 余量；估算是保守上界，不是磁盘预留。
  ZIP 保存及回读校验完成后先释放源捕获副本，再继续加密，避免不必要的副本同时驻留。
  每个成员名最多 180 个 UTF-8 字节，不接受路径、设备名或文件夹。详见 [多文件流程与协议](docs/MULTIFILE_0_4_0.md)。
...
```text
app/                     Android Manifest、SAF／Download 保存、ViewModel、Compose 页面、三主题
...
py -3 .\tools\verify_bundle_python.py
```
...
本轮覆盖多文件临时空间、文件选择取消、设置与当前任务保留、未保存结果确认，以及原有功能回归。
最终执行结果、APK 信息与实际设备边界由 [0.4.1 验证报告](verification/TEST_REPORT_0_4_1.md) 记录；
不能把测试用例已编写、测试 APK 已编译或历史通过数字当作本版设备测试通过。

### 0.4.0-alpha 历史验证

多文件创建、选择恢复、部分保存与取消、双向 ZIP 和容器互通，以及构建结果见
[0.4.0 验证报告](verification/TEST_REPORT_0_4_0.md)。Android 原生界面与文件提供方仍需设备验收。

### 0.3.2-alpha 历史验证

Download 保存、重试、打开方式、已有功能回归和构建的实际结果统一记录于
[0.3.2 验证报告](verification/TEST_REPORT_0_3_2.md)，测试计数与状态以该报告为准。
该版本未进行手机／模拟器验收，待执行项目见 [设备验收清单](docs/DEVICE_ACCEPTANCE.md)。
...
恢复：选择 PNG、GIF 或 SAES → 凭据 → 完整认证并生成私有明文成品 → 自动保存到公共 `Download/` 根目录 →
写入后回读核对长度和 SHA-256 → 显示实际保存名称与位置。已有同名文件保留，新文件使用可用名称。
点击“打开文件”才弹出系统打开方式选择；保存完成不会自动启动其他应用。

保存失败或在保存阶段安全取消时，当前已认证的私有成品保留，可点击“重试保存到下载文件夹”，
也可“选择其他位置保存”，无需重新解密或输入口令。成功后可“另存一份”。清空结果或重新启动应用会清理私有成品，
**已公开保存的 Download 明文仍保留**。未知格式或没有可用查看应用时，文件仍可从保存位置找到，或另存后使用合适的应用打开。
MIME 根据已认证原文件名和最多 4 KiB 文件头推断，不把 1 GiB 文件整体读入内存来识别类型。
完整流程与平台差异见 [恢复保存说明](docs/DOWNLOAD_RESTORE_0_3_2.md)。

进入设置、调整外观或工作预算，再返回当前功能时，文件选择和认证结果保留；当前“只读验证”或“生成密钥”子模式不会重置。
重复点击当前导航页或当前子模式不清理状态。真正切换功能或点击清空结果时，如尚有未保存成品，先询问是否丢弃；
选择保留继续使用原结果，确认丢弃才清理私有文件。已保存的 Download／SAF 文件不受影响。表单口令继续按原隐私策略清理。

独立加密：私有 SAES 成品认证完成后，仍通过系统文件选择器创建新文档并回读校验。
...
没有 INTERNET、全盘访问权限或统计上传。Android 10／API 29 及以上通过应用新建的 MediaStore Download 条目保存，
不申请存储权限；Android 8／9（API 26–28）向公共 Download 保存需要传统存储授权，权限声明仅到 API 28。
拒绝授权后仍可保留当前结果重试，或用 SAF 选择其他位置保存。
云端文档提供方和用户选择的查看应用可能自行联网处理文件，不能把“应用本地计算”理解为“整个系统绝不联网”。

口令不写 SharedPreferences 或 SavedState；任务开始、后台切换等时清空表单值。
JVM String、GC、系统换页无法给出物理内存安全擦除保证。

私有输出使用 `noBackupFilesDir`，备份被禁用。认证未完成的失败任务清理私有输出；已认证的恢复成品在保存失败或安全取消保存后
保留于当前结果，以便重试。确认丢弃、开始新任务、更换输入或下次进程启动会清理私有成品；设置往返不清理当前结果。
没有未保存内容时，正常切换功能或清空结果仍会释放私有副本；不提供进程终止后的恢复保证。
清空结果和卸载本应用不会由本应用删除已经保存的公共 Download 文件，请用文件管理器管理这些明文。删除不是 SSD 物理安全擦除。

API 29 及以上在写入与回读校验期间保持 MediaStore 条目为 pending，完成后才发布。
API 26–28 没有等同的 pending 隔离：其他有存储访问权的应用可能在校验结束前看到部分文件。
两条路径均在完整回读校验后才报告保存成功；失败或提交前取消会尽力清理本次输出，无法清理时提示用户检查。
保存已经提交后到达的取消不会撤回成功文件。打开文件只授予所选应用单个 `content` URI 的临时读取权限，不授予写权限。
...
文件选择取消会向支持取消的文档提供方传递 CancellationSignal，并尝试关闭本次读取句柄；旧选择的迟到结果不会替代新选择。
取消仍是协作式；不承诺立即打断忽略取消、阻塞中的提供方，也不保证立即打断 scrypt 或单次 AES 调用。屏幕默认启用 FLAG_SECURE。
